...
[
    ...
    enemy_1 exists? 0x006BDF
    enemy_2 exists? 0x006BF3
    enemy_3 exists? 0x006C07
    enemy_4 exists? 0x006C1B
    enemy_5 exists? 0x006C2F
    enemy_6 exists? 0x006C43
    enemy_7 exists? 0x006C57
    enemy_8 exists? 0x006C6B
    enemy_9 exists? 0x006C7F
...
* If you need to know which enemies we are fighting in a battle, for all 9 enemy slots, you need to check both whether they exist, and their type. If they do not exist in a given slot, ignore their type, there is no enemy in that slot to speak of. For example, if enemy type for slot 5 is "Imp", but "exists?" for slot 5 is "no", there is no Imp in slot 5 (there is nothing in slot 5).